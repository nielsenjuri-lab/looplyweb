    ...
    <>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          <div style={{ background: '#FFFFFF', borderRadius: 14, padding: '14px', border: '1px solid #E5DDD5' }}>
            <p style={{ color: '#8C8A86', fontSize: 11, marginBottom: 6 }}>Я сдаю</p>
            <p style={{ color: '#4CAF50', fontSize: 18, fontWeight: 700 }}>{summary.available}</p>
            <p style={{ color: '#8C8A86', fontSize: 11 }}>свободно</p>
            <p style={{ color: '#FF6B4A', fontSize: 14, fontWeight: 600, marginTop: 8 }}>{summary.inProgress}</p>
            <p style={{ color: '#8C8A86', fontSize: 11 }}>в процессе</p>
            {summary.notListed > 0 && (
              <>
                <p style={{ color: '#8C8A86', fontSize: 14, fontWeight: 600, marginTop: 8 }}>{summary.notListed}</p>
                <p style={{ color: '#8C8A86', fontSize: 11 }}>не сдаётся</p>
              </>
            )}
          </div>
          <div style={{ background: '#FFFFFF', borderRadius: 14, padding: '14px', border: '1px solid #E5DDD5' }}>
            <p style={{ color: '#8C8A86', fontSize: 11, marginBottom: 6 }}>Я арендую</p>
            <p style={{ color: '#8FA79A', fontSize: 18, fontWeight: 700 }}>{renterBookings.length}</p>
            <p style={{ color: '#8C8A86', fontSize: 11 }}>активных заявок</p>
            <Link href="/bookings" style={{
              display: 'inline-block', marginTop: 12,
              color: '#FF6B4A', fontSize: 12, fontWeight: 600,
            }}>
              Все аренды →
            </Link>
          </div>
              ...
              return (
                <Link
                  ...
                  style={{
                    ...
                    background: '#FFFFFF', borderRadius: 12, padding: '12px', border: '1px solid #E5DDD5',
                  }}
                >
                  <div style={{ width: 48, height: 48, borderRadius: 10, background: '#EFE8E0', overflow: 'hidden', flexShrink: 0 }}>
                    {item?.image_urls?.[0]
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ color: '#B5AFA9', fontSize: 11, marginTop: 2 }}>
      ...
      <div style={{ marginBottom: 24 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          <h2 style={{ color: '#2B2A28', fontWeight: 600, fontSize: 16 }}>Я сдаю</h2>
        ...
        {myItems.length === 0 ? (
          <div style={{
            ...
            <p style={{ color: '#8C8A86', fontSize: 14 }}>Нет объявлений</p>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {myItems.map((item, index) => {
              ...
              const rowContent = (
                <>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ color: '#FF6B4A', fontSize: 13, marginTop: 2 }}>
                      {item.price_per_day.toLocaleString('ru-RU')} ₽/день
                    </p>
                    {statusInfo.activeBooking && (
                      <p style={{ color: '#8C8A86', fontSize: 11, marginTop: 4 }}>
              ...
              return (
                <div key={item.id}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, background: '#FFFFFF', borderRadius: 12, padding: '12px', border: '1px solid #E5DDD5' }}>
